...
trigger prevent_same_title on Work_Item__c (before insert, before update) {
    for(Work_Item__c wi1 : Trigger.new){
        ...
        List<Work_Item__c> list_wi = [SELECT ID,Name FROM Work_Item__c WHERE Sprint__r.Project__c = :sprint[0].Project__c AND ID != :wi1.ID];
        //COMPARE NAMES
        for(Work_Item__c wi2 : list_wi){
            if(wi1.Name == wi2.Name){
                Trigger.oldMap.get(wi1.Id).addError('The title is already registered in the sprint');
            }
        }
    }
}
